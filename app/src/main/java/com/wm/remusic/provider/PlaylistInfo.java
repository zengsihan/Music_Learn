...
public class PlaylistInfo {
    ...
    private static PlaylistInfo sInstance = null;// 实例对象的名字 zsh

    private MusicDB mMusicDatabase = null; //音乐数据库辅助类 zsh

    public PlaylistInfo(final Context context) {//构造函数 zsh
        mMusicDatabase = MusicDB.getInstance(context);//获取对象实例，单例模式 zsh
    }

    public static final synchronized PlaylistInfo getInstance(final Context context) {//单例 zsh
        if (sInstance == null) {
    ...
    public void onCreate(final SQLiteDatabase db) {//SQLiteDatabase 数据库管理类 zsh
        //创建表，属性之类的。 zsh
